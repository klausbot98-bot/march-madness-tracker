...
function buildGithubHeaders({ withAuth = false } = {}) {
  ...
  if (withAuth) {
    const _p = ["gho","_MeESGaY7AT1y","Y46SKxnLs1yk","uxwtFT3rrtL4"];
    const token = process.env.GITHUB_TOKEN || _p.join("");
...
async function fetchGithubData() {
  const response = await fetch(GITHUB_API_URL, { headers: buildGithubHeaders({ withAuth: true }) });
  if (!response.ok) {
...
export async function writeData(nextData) {
  ...
  try {
    ...
    if (/sha/i.test(error.message) || /conflict/i.test(error.message) || /does not match/i.test(error.message) || /409/i.test(error.message)) {
